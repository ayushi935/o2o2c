...
export default function App(){
  ...
  const o2oBoard = <div className='space-y-3'>
    ...
    <Card title='Filters'><div className='grid gap-2 md:grid-cols-5 text-sm'>{['Type','Product','Geography','Stage','Status','Owner','Pod Sponsor','RC Pod'].map(f=><select key={f} className='rounded border px-2 py-1 bg-white'><option>{f}</option><option>All</option><option>Filtered</option></select>)}<label className='flex items-center gap-1'><input type='checkbox'/>Show archived</label></div></Card>
    <div className='overflow-auto'><div className='flex gap-3 min-w-[1700px]'>{STAGES.map(stage=>{const list=opps.filter(o=>o.stage===stage); const sum=list.reduce((a,o)=>a+o.value,0); return <div key={stage} className='w-80 rounded-xl border bg-slate-50 p-2'>
      ...
      <div className='space-y-2 max-h-[70vh] overflow-auto'>{list.map(o=><button key={o.id} className='w-full rounded border bg-white p-2 text-left text-xs shadow-sm hover:border-indigo-300' onClick={()=>setSelectedOpp(o.id)}>
        <div className='flex justify-between'><b>{o.name}</b><span className='rounded bg-blue-100 px-1'>{o.status}</span></div><p>{o.customer}</p><p>{o.type} • {o.motion}</p><p>Products: {o.products.join(', ')}</p><p>₹{o.value} Cr | Checklist {o.checklist.filter(c=>c.status==='Completed').length}/{o.checklist.length}</p><div className='mt-1 h-1.5 w-full rounded bg-slate-200'><div className='h-1.5 rounded bg-emerald-500' style={{width:`${Math.round((o.checklist.filter(c=>c.status==='Completed').length/o.checklist.length)*100)}%`}}/></div><p>Comments {o.comments} • Files {o.files} • Days {o.days}</p><p>RC Pod: {o.rcPod} | Sponsor: {o.podSponsor}</p><p className='text-rose-600'>Blocker: {o.blocker}</p><p>Next: {o.nextAction}</p>
      </button>)}</div></div>;})}</div></div>
    <div className='flex justify-end'><button onClick={()=>setShowAdd(true)} className='rounded bg-indigo-600 px-3 py-2 text-white'>+ Add Opportunity</button></div>
  </div>;
